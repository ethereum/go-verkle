...
func TestLeafStemLength(t *testing.T) {
	...
	if len(ser) != nodeTypeSize+StemSize+bitlistSize+3*crypto.SerializedPointUncompressedSize {
		t.Fatalf("invalid serialization when the stem is longer than 31 bytes: %x (%d bytes != %d)", ser, len(ser), nodeTypeSize+StemSize+bitlistSize+2*crypto.SerializedPointUncompressedSize)
	}
}
