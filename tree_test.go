...
func TestComputeRootCommitmentOnlineThreeLeaves(t *testing.T) {
	root := New(10)
	root.InsertOrdered(zeroKeyTest, testValue, lg1, nil)
	root.InsertOrdered(fourtyKeyTest, testValue, lg1, nil)
	root.InsertOrdered(ffx32KeyTest, testValue, lg1, nil)
...
func TestComputeRootCommitmentOnlineThreeLeavesDeep(t *testing.T) {
	root := New(10)
	root.InsertOrdered(zeroKeyTest, testValue, lg1, nil)
	root.InsertOrdered(oneKeyTest, testValue, lg1, nil)
	root.InsertOrdered(ffx32KeyTest, testValue, lg1, nil)
...
func TestComputeRootCommitmentOnlineThreeLeavesFlush(t *testing.T) {
	...
	go func() {
		root := New(10)
		root.InsertOrdered(zeroKeyTest, testValue, lg1, flush)
		root.InsertOrdered(fourtyKeyTest, testValue, lg1, flush)
		root.InsertOrdered(ffx32KeyTest, testValue, lg1, flush)
	...
	b.Run("insertOrdered", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			for _, el := range sortedKVs {
				if err := root.InsertOrdered(el.k, el.v, lg1, nil); err != nil {
					b.Fatal(err)
				}
			}
			root.ComputeCommitment(lg1)
